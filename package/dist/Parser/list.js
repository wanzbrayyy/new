...
export const parseDoujinList = ($, site) => {
    const data = [];
    const baseURL = baseURLS[site];
    const currentPage = Number($('.pagination').find('.page.current').text());
    const totalPages = Number(($('.pagination').find('a.last').attr('href') || '').split('page=')[1]);
    ...
    $('.gallery').each((i, el) => {
        ...
        const coverSlug = contentElements.find('a > img').attr('data-src') ||
